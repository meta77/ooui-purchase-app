<template>
  <div class="space-y-6" v-if="product">
    <div class="space-y-4">
      <router-link to="/products" class="inline-flex items-center text-sm font-medium text-gray-500 hover:text-gray-900 transition-colors">
      ...
      <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div class="text-left sm:text-right">
          ...
          <p class="mt-1 text-2xl font-bold text-gray-900">¥{{ formatPrice(product.price) }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      <div class="px-6 py-4 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
        <h3 class="text-base font-semibold text-gray-900">Purchase History (購入履歴)</h3>
        <span class="text-sm text-gray-500">{{ product.history.length }} records</span>
      </div>
      
      <div v-if="product.history.length > 0" class="divide-y divide-gray-200">
        <router-link
        ...
        >
          <div class="flex-1">
            ...
            <p class="text-xs text-gray-500 mt-1">{{ record.purchase.date }}</p>
          </div>
          <div class="flex items-center gap-8 text-right">
            <div class="w-20">
              <p class="text-xs text-gray-500">Qty</p>
              <p class="text-sm font-medium text-gray-900 mt-0.5">{{ record.quantity }}</p>
            </div>
            <div class="w-32 hidden sm:block">
              <p class="text-xs text-gray-500">Purchase Total</p>
              <p class="text-sm text-gray-700 mt-0.5">¥{{ formatPrice(record.purchase.total) }}</p>
            </div>
          </div>
        </router-link>
      </div>
